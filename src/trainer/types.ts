...
export interface TrainerEvaluationResult<TPrediction = unknown>
  extends TrainerPredictionResult<TPrediction> {
  metrics: ModelMetrics;
}
...
export interface TrainerCallbacks<
  TModel = unknown,
  TPrediction = unknown,
  TResult extends TrainerRunResult = TrainerRunResult,
> {
  onFitStart?: (context: TrainerRunContext<TModel>) => void | Promise<void>;
  onFitEnd?: (context: TrainerRunContext<TModel> & { result: TResult }) => void | Promise<void>;
  onEvaluateStart?: (context: TrainerRunContext<TModel>) => void | Promise<void>;
  onEvaluateEnd?: (
    context: TrainerRunContext<TModel> & { result: TrainerEvaluationResult<TPrediction> },
  ) => void | Promise<void>;
  onPredictStart?: (context: TrainerRunContext<TModel>) => void | Promise<void>;
  onPredictEnd?: (
    context: TrainerRunContext<TModel> & { result: TrainerPredictionResult<TPrediction> },
  ) => void | Promise<void>;
}
