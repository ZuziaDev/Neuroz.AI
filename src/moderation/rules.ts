...
function buildKeywordRule(
  ...
  return {
    id,
    name,
    evaluate(context: ModerationRuleContext): ModerationRuleHit[] {
      const hits = keywords
...
export function createBuiltinModerationRules(): ModerationRule[] {
  return [
    buildKeywordRule(
      "rule-harassment-keywords",
      "Harassment keywords",
      "harassment",
      ["idiot", "stupid", "moron", "salak", "aptal"],
      "harassment_keyword",
      0.92,
    ),
    buildKeywordRule(
      ...
      "Hate keywords",
      "hate",
      ["i hate you", "nefret ediyorum", "hate you"],
      "hate_keyword",
      0.95,
    ),
    buildKeywordRule(
      ...
      "Spam keywords",
      "brand/spam",
      ["buy now", "limited offer", "free money", "hemen tikla"],
      "spam_keyword",
      0.86,
    ),
    buildKeywordRule(
      ...
      "Gambling keywords",
      "brand/gambling",
      ["casino", "bet now", "sportsbook", "bahis"],
      "gambling_keyword",
      0.84,
    ),
    buildKeywordRule(
      ...
      "Drugs keywords",
      "brand/drugs",
      ["cocaine", "meth", "buy weed", "uyusturucu"],
      "drugs_keyword",
      0.9,
    ),
    buildKeywordRule(
      "rule-brand-extremism",
      "Extremism keywords",
      "brand/extremism",
      ["join our extremist group", "terror cell", "extremist manifesto"],
      "extremism_keyword",
      0.97,
    ),
    buildKeywordRule(
      ...
      "Politics keywords",
      "brand/politics",
      ["election", "vote for", "political campaign", "secim"],
      "politics_keyword",
      0.62,
    ),
    buildKeywordRule(
    ...
    buildKeywordRule(
      ...
      "Self-harm keywords",
      "self-harm",
      ["kill myself", "hurt myself", "kendime zarar"],
      "self_harm_keyword",
      0.98,
    ),
    buildKeywordRule(
    ...
    buildKeywordRule(
      ...
      "Phishing keywords",
      "brand/phishing",
      ["verify your account", "reset your bank password", "wallet seed phrase"],
      "phishing_keyword",
      0.95,
    ),
    buildKeywordRule(
      ...
      "Scam keywords",
      "brand/scam",
      ["guaranteed return", "double your money", "send crypto first"],
      "scam_keyword",
      0.94,
    ),
    {
      ...
      evaluate(context: ModerationRuleContext): ModerationRuleHit[] {
        ...
        const ibanMatches = new Set<string>();
...
export class ModerationRuleEngine {
  ...
  public constructor(rules: readonly ModerationRule[] = createBuiltinModerationRules()) {
    this.rules = [...rules];
  }

  public async evaluate(context: ModerationRuleContext): Promise<ModerationRuleResult> {
    const hits = (
      await Promise.all(this.rules.map((rule) => Promise.resolve(rule.evaluate(context))))
    ).flat();
    ...
    return {
      ...
      categories: Object.fromEntries(
        Object.entries(categoryScores).map(([category, score]) => [category, score >= 0.5]),
      ),
      categoryScores,
