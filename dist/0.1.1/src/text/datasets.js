...
function ensurePositiveSequenceLength(sequenceLength) {
...
export function buildCausalLanguageModelDataset(texts, tokenizer, options) {
    ...
    for (const text of texts) {
        ...
        for (let start = 0; start + options.sequenceLength < tokens.length; start += stride) {
...
export function buildModerationDataset(examples, tokenizer, options) {
    ensurePositiveSequenceLength(options.sequenceLength);
    const labels = options.labels ?? [...new Set(examples.map((example) => example.label))].sort();
    const normalizedLabels = labels.filter((label) => typeof label === "string");
    const labelToIndex = new Map(normalizedLabels.map((label, index) => [label, index]));
    function activeLabelsForExample(example) {
        ...
        if (example.labels !== undefined) {
            return example.labels.filter((label) => label !== options.safeLabel && labelToIndex.has(label));
        }
    ...
    function multiHot(activeLabels) {
    ...
    const dataset = new Dataset(examples.map((example) => {
        const activeLabels = activeLabelsForExample(example);
        const categories = Object.fromEntries(normalizedLabels.map((label) => [label, activeLabels.includes(label)]));
        return {
