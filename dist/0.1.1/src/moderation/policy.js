...
function thresholdForCategory(thresholds, category, fallback) {
    return thresholds?.categories?.[category] ?? thresholds?.default ?? fallback;
}
export function createModerationPolicyProfile(mode, overrides = {}) {
    const reviewMargin = overrides.reviewMargin ?? (mode === "strict" ? 0.15 : 0.08);
    const defaultThreshold = mode === "strict" ? 0.45 : mode === "relaxed" ? 0.7 : mode === "custom" ? 0.5 : 0.55;
    const defaultAction = mode === "strict" ? "review" : mode === "relaxed" ? "annotate" : "review";
    return {
...
export function createModerationPolicyFromDsl(id, version, dsl) {
...
export function mergeModerationSources(modelEntry, rules, applyHierarchy) {
    ...
    return {
        ...
        category_applied_input_types: Object.fromEntries(Object.keys(applied.categoryScores).map((category) => [
            category,
            applied.categories[category] ? ["text"] : [],
        ])),
    };
}
export function resolveModerationDecision(entry, policy, reasonCodes, riskScore = 0) {
    ...
    for (const [category, rawScore] of Object.entries(entry.category_scores)) {
        ...
        const reviewThreshold = actionConfig?.reviewThreshold ?? Math.max(threshold - (policy.reviewMargin ?? 0.08), 0);
        if (score >= threshold) {
    ...
    else if (riskScore >= (policy.risk?.reviewScore ?? Number.POSITIVE_INFINITY) &&
        action === "allow") {
        action = "review";
        needsReview = true;
    }
    const status = action === "allow" || action === "annotate" || action === "redact"
        ? entry.flagged
            ? "flagged"
            : "allowed"
