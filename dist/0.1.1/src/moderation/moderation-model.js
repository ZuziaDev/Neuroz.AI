...
import { DEFAULT_NEUROZAI_OPENAI_MODERATION_MODEL, OPENAI_MODERATION_CATEGORIES, } from "./openai-schema.js";
...
function argmax(values) {
    return values.reduce((bestIndex, value, index, items) => value > items[bestIndex] ? index : bestIndex, 0);
}
function deriveDefaultCategories(options) {
...
export class ModerationModel {
    ...
    static async load(store, registry, record) {
        ...
        const config = await store.readJson(path.join(artifactDir, MODERATION_CONFIG_FILE));
        ...
        for (const token of tokens) {
            ...
            for (const category of candidateCategories) {
                tokenContributions.push({
                    token,
                    category,
                    contribution: Number(((base.category_scores[category] ?? 0) - (ablated.category_scores[category] ?? 0)).toFixed(6)),
                });
            }
        }
        tokenContributions.sort((left, right) => right.contribution - left.contribution);
        return {
            summary: tokenContributions.length === 0
    ...
    requireModel() {
        if (this.tfjsModel === undefined) {
            throw new ModelNotReadyError("Moderation model is not initialized. Train it first or load a saved version.");
        }
        return this.tfjsModel;
    }
    resolveCategoriesFromExamples(examples) {
        ...
        if (categories.length === 0) {
            throw new ConfigurationError("No moderation categories were discovered. Provide labels/categories or configure labels explicitly.");
        }
        return categories;
    }
    async predictEntry(text) {
        const model = this.requireModel();
        ...
        const categories = Object.fromEntries(this.categories.map((category, index) => {
            const score = rawScores[index] ?? 0;
            return [category, score >= thresholdForCategory(this.thresholds, category)];
        }));
        const categoryScores = Object.fromEntries(this.categories.map((category, index) => [category, rawScores[index] ?? 0]));
        const categoryAppliedInputTypes = Object.fromEntries(this.categories.map((category) => [
            category,
    ...
    async createTfjsModel() {
        if (this.categories.length === 0) {
            throw new ConfigurationError("Moderation model labels are empty. Provide labels or train with labeled examples first.");
        }
...
//# sourceMappingURL=moderation-model.js.map